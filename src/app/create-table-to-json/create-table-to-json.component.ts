...
export class CreateTableToJsonComponent implements OnInit {
  ...
  ngOnInit() {
    const database = DatabaseEngine.getDatabaseEngines();
    this.sql = database;
  ...
  public setDDL(database) {
    const index = this.sql.findIndex(item => {
  ...
  public createTable() {
    const ct = new CreateTableToJsonService();
    ...
    if (!ct.hasError()) {
      ...
      schemas = schemas.map(schema => {
        const bootstrapGridSystem = new BootstrapGridSystemService(schema.data, `${this.gridModel.value}\n`);
