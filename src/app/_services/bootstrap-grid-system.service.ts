...
export class BootstrapGridSystemService {
  ...
  convert() {
    const groups = [];
    const grid = this._grid.replace(/ +/g, ' ').trim();
    const arrGrid = grid.split(' ');

    const chunkSize = arrGrid.length;
    ...
    const lastRow = this._page.rows.length;
    const columns = this._page.rows[lastRow - 1].columns;
